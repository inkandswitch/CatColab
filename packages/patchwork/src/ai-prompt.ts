        ...
        edit: async (
            ...
            handle.change((doc) => {
                ...
                const findCellIndex = (idOrName: string): number => {
                    ...
                    let index = doc.notebook.cells.findIndex(
                        (c) => c.id === idOrName
                    );
                    ...
                    if (cellId) {
                        index = doc.notebook.cells.findIndex(
                            (c) => c.id === cellId
                        );
                        if (index >= 0) return index;
                    }
                ...
                for (const op of otherOps) {
                    ...
                    switch (op.type) {
                        ...
                        case "delete-cell":
                            ...
                            if (deleteIndex !== -1) {
                                const cellToDelete =
                                    doc.notebook.cells[deleteIndex];
                                validateAndLogId(
                                    cellToDelete!.id,
                            ...
                            const deletedCount =
                                initialLength - doc.notebook.cells.length;
                            console.log(
